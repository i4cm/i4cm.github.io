...
function card(code,h){
  if(!h) return '<div class="card"><span class="code">'+esc(code)+'</span>: not listed - try another code or see the table below.</div>';
  var p='<div class="card"><span class="code">Error '+esc(code)+'</span> <small>'+esc(h.cat)+'</small>';
  if(h.msg)  p+='<div class="label">Reported as</div><div>'+esc(h.msg)+'</div>';
  if(h.self) p+='<div class="label">Official fix (software)</div><div>'+esc(h.self)+'</div>';
  if(h.hw)   p+='<div class="label">Repair-level cause</div><div>'+esc(h.hw)+'</div>';
  if(h.tip)  p+='<div class="label">How to tell</div><div>'+esc(h.tip)+'</div>';
  if(h.scene)p+='<div class="label">Real-world case</div><div>'+esc(h.scene)+'</div>';
  return p+'</div>';
}
...
function table(){
  ...
  document.getElementById("tbl").innerHTML='<tr><th>Code</th><th>Official category</th><th>Official fix</th><th>Repair-level cause</th></tr>'+rows;
}
